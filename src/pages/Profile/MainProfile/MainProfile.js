        ...
        <div className='profile-bio'>
          {
            <div >
              ...
              <div className='avatar-img'>
                ...
                <div className='userInfo' style={{'backdropFilter':'blur(3px)'}}>
                  <div>
                    <h3 className='heading-3'>
                      {loggedInUser[0]?.name ? loggedInUser[0].name : user && user.displayName}
                    </h3>
                    <p className='usernameSection'>@{username}{blue?<VerifiedUserIcon className="post__badge" />:''}</p>
                  </div>
                  <EditProfile user={user} loggedInUser={loggedInUser}/>
                </div>
                <div className='infoContainer' style={{'backdropFilter':'blur(3px)'}}>
                  {loggedInUser[0]?.bio ? <p>{loggedInUser[0].bio}</p> : ''}
                  <div className='locationAndLink'>
                    {loggedInUser[0]?.location ? <p className='subInfo'><MyLocationIcon /> {loggedInUser[0].location}</p> : ''}
                    {loggedInUser[0]?.website ? <p className='subInfo link' onClick={handleurl}><AddLinkIcon /> {loggedInUser[0].website}</p> : ''}
                  </div>
                </div>
